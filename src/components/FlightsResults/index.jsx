...
function FlightsResults({ flights }) {
  return (
    <div>
      <Typography variant="h6" sx={{ mt: 4 }}>Top flights</Typography>
            ...
            <Accordion key={flight.id} >
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', width: '20%' }}>
                  <Box sx={{ m: 2 }}>
                    <img src={logoUrl} width={35} height={35} alt="air company logo" />
                  </Box>
                  <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
                      <Typography variant="subtitle1">{dayjs(flight.legs[0].departure).format('hh:mm A')} -</Typography>
                      <Typography variant="subtitle1">{dayjs(flight.legs[lastLeg].arrival).format('hh:mm A')}</Typography>
                    </Box>
                ...
                <Box sx={{ width: '20%', textAlign: 'center' }}>
                  ...
                  <Typography>{flight.legs[0].destination.displayCode}-{flight.legs[lastLeg].destination.displayCode}</Typography>
                </Box>
                ...
                <Box sx={{ width: '20%', textAlign: 'center' }}>
                  <Typography>{formatCO2(flight.eco?.ecoContenderDelta)}</Typography>
                </Box>
